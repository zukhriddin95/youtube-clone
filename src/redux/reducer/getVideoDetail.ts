...
export const getVideoDetails = createAsyncThunk(
  ...
  async (id: string) => {
    const {
      ...
      `${YOUTUBE__API__URL}/videos?key=${API_KEY}&part=snippet,statistics&type=video&id=${id}`
    );
...
const parseData = async (item: {
  ...
  const {
    ...
    `${YOUTUBE__API__URL}/channels?part=snippet,statistics&id=${item.snippet.channelId}&key=${API_KEY}`
  );
